...
#SBATCH --output=mgtr_denoise_%A_%a.log
#SBATCH --error=mgtr_denoise_%A_%a.err
...
# Input files
me_denoised="$base_path/completed_multiecho/sub-${subject}/ses-${session}/sub-${subject}_ses-${session}_desc-medenoised_space-T1w_bold.nii.gz"
gm_probseg="$base_path/fmriprep/sub-${subject}/ses-${session}/anat/sub-${subject}_ses-${session}_run-001_label-GM_probseg.nii.gz"

# Output directory
output_dir="$base_path/completed_preprocessing/multiecho/sub-${subject}/ses-${session}"
mkdir -p "$output_dir"
...
fsl_regfilt -i "$me_denoised" \
            ...
            -o "$output_dir/sub-${subject}_ses-${session}_task-rest_space-T1w_desc-medenoisedmgtr_bold.nii.gz" \
            -f "1"
