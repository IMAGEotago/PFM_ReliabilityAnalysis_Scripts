#!/bin/bash
#SBATCH --job-name=ciftify_subject_proc_me
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=48:00:00
#SBATCH --output=ciftify_subject_me_%A_%a.log
#SBATCH --error=ciftify_subject_me_%A_%a.err
...
fmri_input_dir="$base_path/completed_preprocessing/multiecho/sub-${subject}/ses-${session}"
fmri_file="$fmri_input_dir/sub-${subject}_ses-${session}_task-rest_space-T1w_desc-medenoisedmgtr_bold.nii.gz"
ciftify_dir="$base_path/ciftify_me"
task_label="sub-${subject}_ses-${session}_task-rest_space-T1w_desc-medenoisedmgtr_bold"
